...
#
# 用途：第 3、4 章（第三、四次培训）备课。造大纲点名要的讲师侧道具：
...
#   ./make-fixtures.sh --out ../fixtures --corrupt garbage # Dify 太宽容时换这个
#
...
#       所以 Dify **有可能**也能解析出部分文本。第 4 章首次实操时务必实测，
#       若 Dify 照单全收，换 garbage 模式。
...
PY
print "      ⚠️ 最终验证只能在 Dify 上做（第 4 章）：能上传、解析失败 = 合格。"
print "         若 Dify 照单全收解析出了文本，重跑：--corrupt garbage"
...
print ""
print "清单："
print "  cases/         → 第 3 章（案例收集智能体演示的输入，给没带作业的学员）"
print "  project-pack/  → 第 4 章（循环节点的 5 份资料，含 1 份损坏件）"
print "  templates/     → 第 4 章（多分支的三专业模板）"
print "  schema/        → 第 3 章（案例卡片结构化输出契约）"
